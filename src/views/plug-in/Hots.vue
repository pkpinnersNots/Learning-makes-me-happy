<template>
    <div class="hot">
        <van-nav-bar title="都市爽文" left-arrow @click-left="onleft" class="nav"/>
...
export default {
    ...
    created () {
        var url = '/api/book/by-categories?gender=male&type=hot&major=%E9%83%BD%E5%B8%82&minor=&start=0&limit=20'
        console.log(url)
        this.$axios.get(url).then(res => {
            console.log(res.data)
            res.data.books.forEach(element => {
                element.cover = str + element.cover
                console.log("联合成：", element.cover)
            });
